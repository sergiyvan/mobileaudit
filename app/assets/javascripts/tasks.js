...
function taskStepNumerize(){
  ...
  while(++key<max){
    domNumer = domSteps[key].getElementsByClassName('task_step_numer')[0];
...
function taskTypeTurn( dom ){
  ...
  var domAnswerS = domStep.getElementsByClassName('task_step_answers')[0];
...
function taskStepPut(dom){
  var mold = '\
                ...
                <div class="task_step_numer">Номер шага: 0</div>\
                <div>Тип подзадачи:\
                ...
                <div class="task_step_answers">Предопределенные ответы:<br />\
